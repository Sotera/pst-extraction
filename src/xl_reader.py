...
def json_transform(xml_doc):
    xml_doc.update({
        ...
        "bccs": [],
        "bccs_line": [],
        "body": u"".join(u"Applicant:  {}, Country:  {}\n".format(xml["name"], xml["country"]) for xml in xml_doc["applicants"])
                + u"\nAbstract:  \n{}".format(xml_doc.get("abstract")),
        "categories": [],
        "ccs" : [],
        "ccs_line" : [],
        # "ccs": [xml["name"] for xml in xml_doc["applicants"]],
        # "ccs_line": [";".join(xml["name"] for xml in xml_doc["applicants"])],
