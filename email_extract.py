...
def createRow(email_id, _dir, mail, attach, msg_body):
    ...
    body = clean_string(quopri.decodestring(msg_body),
                     [            
                         ...
                         EXPR_OPTS['fix_newline'], 
                         EXPR_OPTS['fix_cr']])
    return "\t".join([email_id, _dir, 
                      "", #scolon_sep(categories), 
                      dateToUTCstr(head(mail_date)) if mail_date else 'NODATE' , '', addr_tostr(senders), '', addr_tostr(tos), addr_tostr(ccs), addr_tostr(bccs), scolon_sep(attach), one(msgid), csv_sep(inreplyto), scolon_sep(references), subject, body])

# in: email as string
# out: map of meta information
# side_effect: write email to out_dir 
#     along with attachments
def extract(email_id, message, out_dir):
    _dir = "{}/emails/{}".format(out_dir, email_id)
    mkdirp(_dir)
    #write raw email to new dir
    spit("{}/{}.eml".format(_dir, email_id), str(message))
    ...
    row= createRow(email_id, "emails/{}".format(email_id), message, attach, msg)
